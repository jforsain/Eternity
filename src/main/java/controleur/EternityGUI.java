...
public class EternityGUI extends JFrame implements Serializable, MouseListener,
	...
	private ImageIcon img = new ImageIcon(
			"src/main/resources/icon_eternity.jpg");

	private Case iscliqued;
	private String vueIsCliqued;

	public EternityGUI(PlateauDeJeuModele pPlateauDeJeuModele,
			PlateauDeJeuVue pPlateauDeJeuVue, ChoisirPiecesVue pChoisirPiecesVue) {
		...
		this.info = new Informations(clearAll, solution, newGame);
		...
		this.sauvegarderJeuPanel = new Savegame(ouiTextfieldSaveGame,
				annulerTextfieldSaveGame);
		this.aPropos = new AboutPanel(this.okApropos);

		this.jSplitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,
				plateauDeJeuVue, choisirPiecesVue);
	...
	public void initialisationFenetre() {
		...
		/* KEY LISTENERS */
		this.setFocusable(true); // Ajout du focus pour intercepter les saisies
									// claviers
		...
		this.getContentPane().add(this.titre, BorderLayout.CENTER); // Ajout de
																	// l'écran
																	// de titre
																	// dans le
																	// panel
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == nouveau || e.getSource() == this.newGame) {
			...
			if (this.plateauDeJeuModele.getPartieEnCours()) {
				...
				int val = JOptionPane.showConfirmDialog(null,
						"Une partie est en cours. Êtes-vous sûr ?", "New Game",
			...
			} else // La partie commence
			{
				...
				this.plateauDeJeuModele.randomShuffleArray();
				this.plateauDeJeuModele.initialiserTerrain();
				this.soundClip.setClip("pushing_onwards.wav");
				this.soundClip.getClip().setFramePosition(0);
				this.soundClip.getClip().loop(Clip.LOOP_CONTINUOUSLY);
				this.info.getTimerLabel().timerStart();
				...
				this.getContentPane()
						.add(this.jSplitPane2, BorderLayout.CENTER);
				this.pack();
			}
		}
		...
		if (e.getSource() == closeGame) // Clique sur 'Fermer'
		{
			if (this.plateauDeJeuModele.getPartieEnCours()) {
				...
				case JOptionPane.NO_OPTION:
				case JOptionPane.CLOSED_OPTION:
					if (!this.plateauDeJeuModele.getPartieEnPause()) // Si la
																		// partie
																		// n'est
																		// pas
																		// en
		...
		if (e.getSource() == this.info.getSolution()) // Clique sur 'Solution'
		{
			...
			this.info.getTimerLabel().timerStop();
			int val = JOptionPane.showConfirmDialog(null,
					"Souhaitez-vous vraiment abandonner ?", "Solution",
					JOptionPane.YES_NO_OPTION);
			switch (val) {
			case JOptionPane.YES_OPTION:
				this.soundClip.setClip("smb_mariodie.wav");
				this.soundClip.getClip().start();
				this.plateauDeJeuModele.setGiveup(true);
				this.plateauDeJeuModele.setPlateau(this.plateauDeJeuModele
						.getCases()); // On fournit la solution
				this.jSplitPane.setRightComponent(this.gameover);
				if (this.plateauDeJeuModele.getPartieEnPause()
						&& this.plateauDeJeuModele.getGiveup()) // Si
																// l'utilisateur
																// à mis le jeu
																// en pause ET
																// qu'il a
		...
		if (e.getSource() == this.stopGame) // Clique sur 'Arrêter'
		{
			...
			this.info.getTimerLabel().timerStop();
			int val = JOptionPane.showConfirmDialog(null,
					"Retourner à l'écran de titre ?", "Stop Game",
					JOptionPane.YES_NO_OPTION);
			switch (val) {
			case JOptionPane.YES_OPTION:
		...
		if (e.getSource() == this.loadGame || e.getSource() == this.saveGame) // Clique
																				// sur
																				// 'Charger'
		{
			if (this.plateauDeJeuModele.getPartieEnCours()) {
		...
		if (e.getSource() == this.okGame) // Clique sur OK selon que l'on ait
											// cliqué sur 'Charger partie' ou
											// 'Sauvegarder partie'
		{
			if (this.gamePanel.getTitle().equals("Sauvegarder partie")
					&& this.gamePanel.getjList().getSelectedValue() != null) {
				this.gamePanel.setEnabled(false);
				this.sauvegarderJeuPanel.setVisible(true);
				this.sauvegarderJeuPanel.setTextField(this.gamePanel.getjList()
						.getSelectedValue().toString());
				;
			}
			if (this.gamePanel.getTitle().equals("Charger partie")
					&& this.gamePanel.getjList().getSelectedValue() != null) {
				if (this.plateauDeJeuModele.chargerPartie(this.gamePanel
						.getjList().getSelectedValue().toString())) {
					this.sauvegarderJeuPanel.setVisible(false);
					this.gamePanel.setEnabled(true);
					this.gamePanel.setVisible(false);
					this.setEnabled(true);
					if (!this.plateauDeJeuModele.getPartieEnCours()) // Si c'est
																		// l'écran
																		// de
																		// titre
					{
		...
		if (e.getSource() == this.annulerGame) { // Clique sur 'Annuler'
			this.gamePanel.setVisible(false);
			this.setEnabled(true);
			if (this.plateauDeJeuModele.getPartieEnCours()) {
				this.info.getTimerLabel().timerStart();
		...
		if (e.getSource() == this.ouiTextfieldSaveGame) // Clique sur 'OK' quand
														// le texte est rentré
		{
			if (this.plateauDeJeuModele
					.sauvegarderPartie(this.sauvegarderJeuPanel.getTextField()
							.getText())) {
				TextFileWriter.append(this.sauvegarderJeuPanel.getTextField()
						.getText());
				...
				this.info.getTimerLabel().timerStart();
				this.soundClip.getClip().loop(Clip.LOOP_CONTINUOUSLY);
				this.gamePanel = new LoadGame(okGame, annulerGame, ""); // On
																		// rafraîchit
																		// la
																		// JList
			} else {
				...
				JOptionPane.showMessageDialog(null,
						"Le nom de fichier existe déjà.", "Erreur sauvegarde",
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == 115 && this.plateauDeJeuModele.getPartieEnPause()) // Touche
																					// F4
		{
			this.soundClip.getClip().loop(Clip.LOOP_CONTINUOUSLY);
			this.info.getTimerLabel().timerStart();
		...
		if (e.getKeyCode() == 27 && !this.plateauDeJeuModele.getPartieEnPause()) // Touche
																					// Esc
		{
			this.soundClip.getClip().stop();
			this.info.getTimerLabel().timerStop();
		...
		if ((e.getKeyCode() == 113 && this.plateauDeJeuModele
				.getPartieEnCours()) || e.getKeyCode() == 114) // Touches F2 ou
																// F3
		{
			if (this.plateauDeJeuModele.getPartieEnCours()) {
				this.soundClip.getClip().stop();
		...
		if (plateauDeJeuModele.getPartieEnCours()
			...
			if (SwingUtilities.isRightMouseButton(e) || e.isControlDown()) {
				...
				if (vue == "vue.PlateauDeJeuVue")
					...
					tmpPiece = (Piece) this.plateauDeJeuModele
							.getCasesShuffle()[vertical][horizontal];
				...
				if (iscliqued == null) {
					try {
						...
						if (vue == "vue.PlateauDeJeuVue") {
							this.iscliqued = this.plateauDeJeuModele
									.getPlateau()[vertical][horizontal];
			...
			if (this.plateauDeJeuModele.isGameEnded()) {
				...
				pause.setEnabled(false);
				this.plateauDeJeuModele.setPartieEnCours(false);
				this.jSplitPane.setRightComponent(this.victoire);
